...
select sq.* from (select store_id, rating, count(f.film_id) as movieCount, 
	rank() over (partition by store_id order by count(f.film_id) desc) as Row_Rank from film f 
	join inventory i on f.film_id=i.film_id group by store_id, rating ) sq; --(to get all the ratings)

select sq.* from (select store_id, rating, count(f.film_id) as movieCount, 
	rank() over (partition by store_id order by count(f.film_id) desc) as Row_Rank from film f j
	oin inventory i on f.film_id=i.film_id group by store_id, rating ) sq having sq.Row_Rank = 1; --(only the highest)
...
from film f 
	...
	left join language l on f.language_id  = l.language_id limit 10;
 

--Q8. How many times each movie has been rented out?

select f.film_id, f.title, count(rental_id) rentedCount 
from film f 
...
select sq.* from 
	(select c.customer_id, c.first_name, c.last_name, c.email, SUM(p.amount) cutomerSpent, 
		RANK() OVER (ORDER BY SUM(p.amount) desc) AS RowRank 
		FROM customer c 
			left join payment p on c.customer_id = p.customer_id 
			where c.active = 1 
		group by c.customer_id, c.first_name, c.last_name, c.email order by 5 desc) sq 
having sq.RowRank = 1;

 --Q11.  What Store has historically brought the most revenue?
 
select s.store_id, sum(p.amount) StoreRevenue 
	from store s 
...
select count(rental_id) RentalCount, Month(rental_date) Month, year(rental_date) Year 
from rental 
...
select count(rental_id) RentalCount, MonthName(rental_date) Month, year(rental_date) Year 
	from rental 
group by Month(rental_date), year(rental_date);
...
select f.film_id, f.title, max(r.rental_date) lastRentalDate, min(r.rental_date) firstRentalDate from film f 
	left join inventory i on f.film_id = i.film_id 
	left join rental r on i.inventory_id = r.inventory_id 
group by f.film_id, f.title limit 10;
 
--Q17: What is the Last Rental Date of every customer?

select c.customer_id, c.first_name, min(rental_date) firstRentalDate, max(rental_date) lastRentalDate from customer c 
	left join rental r on r.customer_id = c.customer_id 
group by c.customer_id, c.first_name limit 10;
 

--Q18: What is our Revenue Per Month?

select monthname(rental_date), sum(amount) RevenuePerMonth from rental r 
	left join payment p on r.rental_id = p.rental_id 
...
select monthname(rental_date) RentalMonth, year(rental_date) RentYear, count(distinct customer_Id) 
	from rental 
...
select monthname(rental_date) RentalMonth, year(rental_date) RentalYear, count(distinct f.film_id) DistinctFilmsRented 
	from rental r 
		left join inventory i on r.inventory_id = i.inventory_id 
		left join film f on i.film_id = f.film_id 
group by monthname(rental_date), year(rental_date) order by 2 desc;
 

--Q21: Number of Rentals in Comedy, Sports, and Family

select count(distinct rental_id) NumRentals, c.name 
from rental r 
	left join inventory i on r.inventory_id = i.inventory_id 
	left join film f on i.film_id = f.film_id 
	left join film_category fc on f.film_id = fc.film_id 
	left join category c on fc.category_id = c.category_id 
group by c.name order by 1 desc;
...
select r.customer_id, c.first_name, c.last_name, count(r.rental_id) NumOfRentals 
from rental r 
...
select  s.store_id,f.rating,sum(amount) 
from store s 
	left join inventory i on s.store_id = i.store_id 
	left join film f on i.film_id = f.film_id 
	left join rental r on i.inventory_id = r.inventory_id 
...
from store s 
	left join inventory i on s.store_id = i.store_id 
	left join film f on i.film_id = f.film_id left join rental r on i.inventory_id = r.inventory_id 
	left join payment p on r.rental_id = p.rental_id 
...
select r.customer_id, c.first_name, c.last_name, c.email, count(r.rental_id) NumOfRentals 
from rental r 
	left join customer c on r.customer_id = c.customer_id 
...
from rental r 
	left join customer c on r.customer_id = c.customer_id 
where c.active = 1 
group by r.customer_id,  c.first_name, c.last_name, c.email having count(r.rental_id)>=30;
 

--Q27: All Rewards Users with Phone

select r.customer_id, c.first_name, c.last_name, c.email, a.phone, count(r.rental_id) NumOfRentals 
from rental r 
	left join customer c on r.customer_id = c.customer_id 
	left join address a on c.address_id = a.address_id 
where c.active = 1 and a.phone is not NULL 
group by r.customer_id,  c.first_name, c.last_name, c.email 
having count(r.rental_id)>=30;
